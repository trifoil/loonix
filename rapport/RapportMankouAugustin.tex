...
Le mode standalone indique que le serveur est autonome, et que le service tourne en permanence. 
    ...
    \item Ils sont chrootés dans /var/ftp/ ;
    \item texttt{/etc/vsftpd/vsftpd\_user\_conf/} sera le dossier pour les configurations d'utilisateurs virtuels ;
    \item texttt{/etc/vsftpd/user\_list} contiendra la liste des utilisateurs refusés (pour lesquels on ne demandera même pas le mot de passe).
	\end{itemize}
